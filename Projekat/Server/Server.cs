...
namespace Server
{
    public class Server : ICommunication
    {
        public void Comunicate()
        {
            ...
            repo.Add(new Common.Models.TeretniBrod(guid, "Test", DateTime.Now, 100, 200, 300, 10, "Utovaruje"), guidBrodogradiliste);
            Console.WriteLine(repo.Get(guid).ToString());
            foreach (var item in repo.GetAll())
            {
                Console.WriteLine(item.ToString());
            }
            repo.Update(new Common.Models.TeretniBrod(guid, "Test Edit", DateTime.Now.AddYears(100), 150, 250, 350, 15, "Istovaruje"));
        }
    }
}
