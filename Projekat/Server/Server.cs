...
namespace Server
{
    public class Server : ICommunication
    {
        public void Comunicate()
        {
            ...
            var repo = new KruzerRepository(new ModelContext());
            var guid = Guid.NewGuid();
            repo.Add(new Common.Models.Kruzer(guid, "Test", DateTime.Now, 100, 200, 300, 10, 20), guidBrodogradiliste);
            Console.WriteLine(repo.Get(guid).ToString());
            foreach (var item in repo.GetAll())
            {
                Console.WriteLine(item.ToString());
            }
            repo.Update(new Common.Models.Kruzer(guid, "Test Edit", DateTime.Now.AddYears(100), 150, 250, 350, 15, 25));
        }
    }
}
